...
class ID3:
    ...
    def entropy(rows: np.ndarray, labels: np.ndarray):
        ...
        counts_prob = {k: v / counts_sum for k, v in counts.items()}
        _entropy = 0.0

        # ====== YOUR CODE: ======
        _entropy = -sum(p * math.log2(p) for p in counts_prob.values())
        # ========================
    ...
    def info_gain(self, left, left_labels, right, right_labels, current_info_gain=None):
        ...
        info_gain_value = current_info_gain - (len(left) / total_size) * left_entropy - (
                    len(right) / total_size) * right_entropy
        # ========================
    ...
    def predict_sample(self, row, node: DecisionNode | Leaf = None):
        ...
        if isinstance(node, Leaf):
            ...
            return node.predictions.keys()[0]
    ...
    def predict(self, rows):
        ...
        # ====== YOUR CODE: ======
        y_pred = np.zeros(rows.shape[0])
        for idx, row in enumerate(rows):
            y_pred[idx] = self.predict_sample(row)
        # ========================
